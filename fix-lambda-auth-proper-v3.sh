#!/bin/bash
# Proper fix: Add API key authentication to Lambda - V3 with secret handling
...
echo "============================================"
echo "ADD API KEY AUTH TO LAMBDA (V3)"
echo "============================================"
echo ""
...
# Check if A2A_API_KEYS_JSON exists in secrets
HAS_SECRET=$(jq -r '
  .containerDefinitions[] |
  select(.name == "orchestrator") |
  .secrets // [] |
  map(select(.name == "A2A_API_KEYS_JSON")) |
  length
' current_orch_taskdef.json)

if [ "$HAS_SECRET" -gt 0 ]; then
  echo "⚠️  Found A2A_API_KEYS_JSON in secrets, removing it..."
fi

# Update task definition - remove from secrets AND environment, then add to environment only
