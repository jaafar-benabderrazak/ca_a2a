#!/bin/bash
# Proper fix: Add API key authentication to Lambda - V4 with better field handling
...
echo "============================================"
echo "ADD API KEY AUTH TO LAMBDA (V4)"
echo "============================================"
echo ""
...
# Create API keys JSON - properly escaped
...
# Use a more conservative approach: only modify environment variables
    ...
    containerDefinitions: [
      ...
      if .name == "orchestrator" then
        . + {
          ...
          secrets: (
            if .secrets then
              [.secrets[] | select(.name != "A2A_API_KEYS_JSON")]
            else
              []
            end
          )
...
MISSING=$(jq -r '
  if .containerDefinitions[0].image == null then "image" 
  elif .containerDefinitions[0].name == null then "name"
  else "ok"
  end
' updated_orch_taskdef.json)

if [ "$MISSING" != "ok" ]; then
  echo "❌ Error: Missing critical field: ${MISSING}"
  echo ""
  echo "Task definition preview:"
  jq '.containerDefinitions[0] | {name, image, cpu, memory, environment: .environment[:3]}' updated_orch_taskdef.json
  exit 1
fi
...
if [ $? -ne 0 ]; then
  ...
  echo "$NEW_TASK_DEF"
  echo ""
  echo "Debug: Container definition:"
  jq '.containerDefinitions[0]' updated_orch_taskdef.json
  exit 1
fi
...
aws logs tail "/aws/lambda/${LAMBDA_NAME}" \
  ...
  --region "${REGION}" \
  --format short \
  | grep -E "Processing|Orchestrator|Success|Error|API|auth|200|401" \
  | tail -25
